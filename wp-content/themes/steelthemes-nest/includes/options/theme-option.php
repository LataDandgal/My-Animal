...
if(!empty($nest_theme_mod['theme_setttings_enable']) == true):
    ...
    /*--=========page header padding============--*/
    $pageheader_padding = '';
    if(!empty($nest_theme_mod['pageheader_padding'])){
        $pageheader_padding = $nest_theme_mod['pageheader_padding'];
    }
    
    $page_header_bg_css = $pageheader_padding ? 'padding:' . $pageheader_padding . ' ' : '';
...
/*------=======peoduct image size ======------*/
    // image
    $image_size_enable = isset( $nest_theme_mod['image_size_enable'] ) ? $nest_theme_mod['image_size_enable'] : '';
    if($image_size_enable == true):
        $pro_image_size = isset( $nest_theme_mod['pro_image_size'] ) ? $nest_theme_mod['pro_image_size'] : '';
        if(!empty($pro_image_size)):
            $customize_css .= '@media(min-width:768px){body .product-img-zoom img , .product-img-action-wrap  img , .product_style_five .product-img-inner a img ,
                .product_grid_style_two .woocommerce-product-gallery__wrapper img {height:'.$pro_image_size.'!important; width:auto!important; margin-left:auto!important; margin-right:auto!important;}}';
        endif;
        $mdpro_image_size = isset( $nest_theme_mod['mdpro_image_size'] ) ? $nest_theme_mod['mdpro_image_size'] : '';
         if(!empty($mdpro_image_size)):
            $customize_css .= '@media(max-width:768px){body .product-img-zoom img ,  .product-img-action-wrap img , .product_style_five .product-img-inner a img ,
                .product_grid_style_two .woocommerce-product-gallery__wrapper img {height:'.$mdpro_image_size.'!important; width:auto!important; margin-left:auto!important; margin-right:auto!important;}}';
        endif;
    endif;
    //  title
    $product_title_resize = isset( $nest_theme_mod['product_title_resize'] ) ? $nest_theme_mod['product_title_resize'] : '';
    if($product_title_resize == true):
        $text_over_flow_size = isset( $nest_theme_mod['text_over_flow_size'] ) ? $nest_theme_mod['text_over_flow_size'] : '';
      ...
      if($filter_deep_content_enable == true):
        $filter_alignment = isset( $nest_theme_mod['filter_alignment'] ) ? $nest_theme_mod['filter_alignment'] : '';
        if(!empty($filter_alignment)): 
            $customize_css .= '.nest_filter_box{ justify-content: '.$filter_alignment.'!important; }';
        endif;
