...
function nest_breadcrumb() {
    ...
    $homeLink = esc_url( home_url());
        if(is_home() || is_front_page()):
          if ($showOnHome == 1) echo '<ul class="bread-crumb"><li><a href="' . $homeLink . '">' . esc_html__('Home' , 'steelthemes-nest') . '</a></li></ul>';
        endif;
      
        if(!is_front_page()){
          echo '<ul class="breadcrumb m-auto"><li><a href="' . $homeLink . '"> <i class="fi-rs-home mr-5"></i>' . esc_html__('Home' , 'steelthemes-nest') . ' </a> </li>';
          if (is_category()){
              global $wp_query;
          ...
          elseif(is_post_type_archive('product') || is_tax('product_cat')  || is_tax('product_tag')  || is_tax('brand')){
