...
<div id="page" class="page_wapper hfeed site"> 
	...
	<?php  // header ?>
		...
		<?php if($disable_mobile_menu == false): ?>
		...
		<?php // page-header end ?>
			<div id="content" class="site-content <?php echo get_post_meta(get_the_ID() , 'blog_single_page_header' , true); ?>">
				<?php $container = 'container';
	                if( is_page_template('template-homepage.php') || is_page_template('template-empty.php') || is_page_template('template-fullwidth.php') || is_singular('header') || is_singular('sticky_header') || is_singular('footer')  || is_singular('mega_menu') || is_404()){
				...
				<div class="<?php echo esc_attr($container); ?>">
					<?php
		                ...
		                $row = 'row';
		                if( is_page_template('template-homepage.php') || is_page_template('template-empty.php') || is_page_template('template-fullwidth.php') || is_singular('header') || is_singular('sticky_header') || is_singular('footer')  || is_singular('mega_menu') || is_404() || $layout_row == 'no-sidebar'):
			                $row = 'no-row';
                        else:
			                $row = 'row default_row';
                        endif;
